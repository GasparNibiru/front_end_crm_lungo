...
const report = [], acquisitions = [], exportRequests = [], assignments = [], scheduledRequests = [];
let acquired = new Set(['owned']), balance = 20, mode = 'normal', failNext = false, scheduleDisabled = false, whatsappConnected = true;
...
async function main() {
  ...
  try {
    ...
    await page.route('**/api/scheduled/availability', route => { assert.ok(route.request().headers()['x-client-token']);return route.fulfill({ status: scheduleDisabled?503:whatsappConnected?200:409, json: scheduleDisabled?{ok:false,error:'O envio programado está indisponível neste ambiente.'}:whatsappConnected?{ok:true,connected:true}:{ok:false,error:'Conecte seu WhatsApp à plataforma em Meus dados antes de agendar mensagens.'} }); });
    ...
    await page.route('**/api/prospecting/**', async route => {
      ...
      if (url.pathname.endsWith('/team')) return send({ ok: true, brokers: [{ id: 'broker-one', name: 'Ana Corretora' }] });
      if (url.pathname.endsWith('/assignments')) { assignments.push(req.postDataJSON()); return send({ ok: true, assignment: { assigned: 1, charged: 0 } }); }
    ...
    assert.ok((await page.locator('#prCards').innerText()).includes('(21) *****-****')); assert.ok(!(await page.locator('#prCards').innerText()).includes(raw[1].mobile_1));
    ...
    await page.locator('#prStateSummary').click(); await page.getByRole('button', { name: 'RJ', exact: true }).click(); await page.locator('#prCategoriesSummary').click(); await page.locator('#prCategories input').nth(0).check(); await page.locator('#prCategories input').nth(1).check(); await page.locator('#prCategoriesSummary').click();
    ...
    assert.equal(await page.locator('#prCityChoices button').count(), 28); assert.equal(await page.locator('#prStateChoices button').count(), 28); await page.locator('#prCitySummary').click(); assert.ok(await page.locator('#prCityChoices').evaluate(el => el.scrollHeight > el.clientHeight)); assert.ok((await page.locator('#prCityChoices').evaluate(el => el.getBoundingClientRect().height)) <= 181); await page.getByRole('button', { name: 'Curitiba / PR', exact: true }).click(); assert.equal(await page.locator('#prState').inputValue(), 'PR'); assert.equal(await page.locator('#prCitySummary').innerText(), 'Curitiba / PR'); await page.locator('#prFilters button[type="submit"]').click(); await page.waitForFunction(() => document.querySelector('#prCards').getAttribute('aria-busy') === 'false'); assert.equal(queries.at(-1).searchParams.get('city'), 'Curitiba'); await page.locator('#prClear').click(); await page.waitForFunction(() => document.querySelector('#prCards').getAttribute('aria-busy') === 'false');
    ...
    whatsappConnected=false;await page.getByRole('button',{name:'◷ Agendamento'}).first().click();await page.waitForFunction(()=>document.querySelector('dialog.pr-action-dialog .pr-action-status').textContent.includes('Conecte seu WhatsApp'));assert.ok(await page.getByRole('button',{name:'Salvar programação'}).isDisabled());assert.equal(scheduledRequests.length,1);assert.equal(exportRequests.length,2);await page.getByRole('button',{name:'Cancelar'}).last().click();whatsappConnected=true;
