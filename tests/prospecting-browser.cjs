...
async function main() {
  ...
  try {
    ...
    await page.route('**/api/prospecting/**', async route => {
      ...
      if (url.pathname.endsWith('/interactions')) { interactions.push(req.postDataJSON()); return send({ ok: true, interaction: { interactionId: 'interaction-one' } }); }
