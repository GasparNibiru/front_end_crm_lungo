...
(function exposeLungoAdminApi() {
  ...
  const getTrainings = (adminKey) => request('/api/admin/training-center', { adminKey });
  const createTraining = (payload, adminKey) => request('/api/admin/training-center', { method: 'POST', adminKey, body: payload });
  ...
  const backfillMetaMarketplaceLeads = (adminKey) => request('/api/admin/lead-marketplace/meta/backfill', { method: 'POST', adminKey, body: {} });
  const getBrazilPartners = (adminKey) => request('/api/admin/brazil-partners', { adminKey });
  const createBrazilPartner = (payload, adminKey) => request('/api/admin/brazil-partners', { method: 'POST', adminKey, body: payload });
  const updateBrazilPartner = (id, payload, adminKey) => request(`/api/admin/brazil-partners/${encodeURIComponent(id)}`, { method: 'PATCH', adminKey, body: payload });
  const deleteBrazilPartner = (id, adminKey) => request(`/api/admin/brazil-partners/${encodeURIComponent(id)}`, { method: 'DELETE', adminKey });

  window.LungoAdminApi = Object.freeze({ verifyAdminKey, getDashboard, getOrganizations, getArchivedOrganizations, createSubscription, updateOrganization, changeOrganizationStatus, getSupervisors, getAccesses, createAccess, updateAccess, archiveAccess, changeAccess, renewAccess, invalidateAccess, resendAccessEmail, getFinancial, getFinancialCalendar, getPayment, updatePayment, confirmPayment, getPaymentHistory, getTrainings, createTraining, updateTraining, deleteTraining, getTrainingMetrics, getCampaignMedia, saveCampaignMedia, getLeadMarketplace, updateLeadMarketplaceSettings, adjustLeadCredits, createMarketplaceLead, updateMarketplaceLead, backfillMetaMarketplaceLeads, getBrazilPartners, createBrazilPartner, updateBrazilPartner, deleteBrazilPartner });
})();
