(() => {
  ...
  const helpTopics = [
    ...
    {id:'connect',title:'Meus dados: como conectar meu WhatsApp?',route:'connect',steps:['Abra Meus dados e acesse Conectar WhatsApp.','Gere o QR Code e escaneie pelo WhatsApp do seu celular, em Aparelhos conectados.','Confira se a plataforma mostra a conexão ativa antes de programar mensagens.']},
  ...
  function renderFeature(list){
    ...
    if(mode==='help'){
      ...
      const order=session.supervisor?['connect','broadcast','crm','schedule','business_intelligence','agenda','clients','finance','marketing-ai']:['connect','crm','schedule','clients','business_intelligence','agenda','broadcast'];
      const topics=helpTopics.filter(x=>!x.supervisor||session.supervisor).sort((a,b)=>order.indexOf(a.id)-order.indexOf(b.id));
  ...
  async function open(options){
    ...
    const choices=[['day','Meu dia'],['help','Ajuda com esse sistema'],['sales','Vendas'],['attention','Prioridades'],...(options.supervisor?[['finance','Financeiro'],['team','Minha equipe']]:[]),['market','Dúvidas do mercado'],['chat','Conversar']];
