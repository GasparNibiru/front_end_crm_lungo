(() => {
  ...
  const helpTopics = [
    ...
    {id:'clients',title:'Como acompanhar minha carteira?',route:'clients',steps:['Abra Clientes para consultar contratos, produtos e dados cadastrados.','Mantenha as datas de renovação e o pós-venda atualizados. A secretária usa essas informações nos lembretes.']},
  ...
  function reset(){generation++;controller?.abort();session=null;items=[];snapshot={leads:[]};failedSources=[];financeCache=null;mode='day';loadingData=false;if(dialog?.open)dialog.close();dialog?.remove();dialog=null;document.querySelectorAll('.daily-dot').forEach(n=>n.hidden=true);}
  function renderList(){
  ...
  async function open(options){
    ...
    dialog.innerHTML='<header class="daily-header"><div><span class="daily-eyebrow">SECRETÁRIA DO LUNGO</span><h2 id="dailyTitle">Seu dia, com mais clareza.</h2></div><button type="button" class="daily-close" aria-label="Fechar resumo do dia">×</button></header><div class="daily-welcome"><span class="daily-orb" aria-hidden="true"><i></i><i></i><i></i></span><div><h3></h3><p class="daily-intro">Vamos conferir o que merece sua atenção.</p></div></div><label class="daily-mode-label" for="dailyMode">Como posso ajudar?</label><select id="dailyMode" class="daily-mode"><option value="day">Meu dia e minha carteira</option><option value="help">Ajuda com o sistema</option><option value="sales">Resumo de vendas</option><option value="attention">O que precisa de atenção?</option></select><nav class="daily-tabs" aria-label="Resumo do dia"><button type="button" data-daily-tab="today" aria-pressed="true">Agenda <b>0</b></button><button type="button" data-daily-tab="pending" aria-pressed="false">Pendências <b>0</b></button><button type="button" data-daily-tab="portfolio" aria-pressed="false">Carteira <b>0</b></button></nav><p class="daily-status" role="status"></p><div class="daily-list"></div><footer><span>Agenda: hoje e próximos 7 dias · Renovações: até 30 dias.<br>Até 500 leads próprios. Resumo sem consumo de créditos.</span><button class="daily-refresh" type="button">Atualizar</button></footer>';
    ...
    const select=dialog.querySelector('#dailyMode');if(options.supervisor)select.append(new Option('Situação do financeiro','finance'));select.onchange=()=>{mode=select.value;renderList();};
