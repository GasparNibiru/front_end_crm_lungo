(() => {
  ...
  function renderAccessTokens() {
    ...
    rows.innerHTML=groups.map(group=>`<details class="admin-access-group" open><summary class="admin-access-toggle"><span aria-hidden="true">›</span><div><b>${escapeHtml(group.client.name)}</b><small>${getPlanDefinition(group.client.planId).name} · ${group.principal.profile==="Supervisor"?"Supervisor e equipe":"Acesso individual"} · ${group.accesses.length} ${group.accesses.length===1?'acesso':'acessos'}</small></div><em title="Corretores vinculados">${group.children.length}</em></summary><div class="admin-access-principal"><label>${group.principal.profile==="Supervisor"?"Supervisor / acesso principal":"Acesso principal"}</label>${accessRow(group.principal,true)}</div><div class="admin-access-children">${group.children.length?`<label>Equipe / corretores vinculados</label>${group.children.map(access=>accessRow(access)).join("")}`:'<p>Nenhum corretor vinculado a este supervisor.</p>'}</div></details>`).join("")||'<p class="empty-admin-row">Nenhum acesso cadastrado.</p>';
