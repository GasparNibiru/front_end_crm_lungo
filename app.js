(() => {
  ...
  async function handleTokenAction(button) {
    ...
    if (action === "email") {
      button.disabled=true;try{const result=await window.LungoAdminApi.resendAccessEmail(access.id,adminMasterKey);if(result?.tokenRenewed){await loadAdminRemoteData();renderAdminV2()}toast(`Acesso reenviado para ${access.raw?.email}.${result?.tokenRenewed?' O token antigo foi substituído automaticamente.':''}`)}catch(error){toast(error.message)}finally{if(button.isConnected)button.disabled=false}return;
    }
    if (action === "edit") {
