(() => {
  ...
  async function handleTokenAction(button) {
    ...
    if (action === "email") {
      button.disabled=true;try{await window.LungoAdminApi.resendAccessEmail(access.id,adminMasterKey);toast(`Acesso reenviado para ${access.raw?.email}.`)}catch(error){toast(error.message)}finally{if(button.isConnected)button.disabled=false}return;
    }
    if (action === "edit") {
