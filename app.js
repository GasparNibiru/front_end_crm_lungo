(() => {
  ...
  function renderAccessTokens() {
    ...
    rows.innerHTML = adminData.accesses.map((access) => { const client = adminClient(access.clientId); const canCopy = access.token?.startsWith("LNG-"); const invalid = access.status === "invalid"; const statusAction = access.status === "blocked" ? `<button class="tiny-btn" data-token-action="reactivate" data-id="${access.id}">Reativar</button>` : invalid ? "" : `<button class="tiny-btn" data-token-action="block" data-id="${access.id}">Bloquear</button>`; return `<tr><td><b>${escapeHtml(client?.name || "—")}</b></td><td>${escapeHtml(access.user)}</td><td>${access.profile}</td><td><code>${escapeHtml(access.token)}</code>${canCopy ? ` <button class="tiny-btn" data-token-action="copy" data-id="${access.id}">Copiar</button>` : ""}</td><td>${adminMasterStatus(adminStatusClass(access.status), access.status === "active" ? "Ativo" : access.status === "blocked" ? "Bloqueado" : "Inválido")}</td><td>${formatDate(access.createdAt)}</td><td>${access.lastAccess}</td><td>${formatDate(access.validUntil)}</td><td><div class="admin-master-actions"><button class="tiny-btn" data-token-action="renew" data-id="${access.id}">Redefinir token</button><button class="tiny-btn" data-token-action="${invalid ? "validate" : "invalidate"}" data-id="${access.id}">${invalid ? "Validar" : "Invalidar"}</button>${statusAction}</div></td></tr>`; }).join("");
