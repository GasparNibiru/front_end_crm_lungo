(() => {
  ...
  document.querySelectorAll('.daily-trigger').forEach(button => button.addEventListener('click', () => {
    ...
    window.LungoDailyAssistant?.open({ token, supervisor, context: (supervisor ? document.querySelector('.supervisor-view.active .view.active')?.id?.replace('view-','') || document.querySelector('.supervisor-view.active')?.id?.replace('supervisor-view-','') : document.querySelector('.view.active')?.id?.replace('view-',''))?.replace('business-intelligence','business_intelligence'), navigate: async item => {
      if ((supervisorAccessToken || state.token) !== token) return;
      if(item.kind==='route'){const routes=['connect','crm','business_intelligence','agenda','clients','broadcast'];if(routes.includes(item.route)){if(supervisor)setSupervisorOperation(item.route);else setView(item.route);}else if(supervisor&&['funnel','finance','marketing-ai'].includes(item.route))setSupervisorView(item.route);return;}
  ...
  function supervisorBrokerAvatar(broker) {
    const photo=String(broker.photo||'');
    const valid=/^data:image\/(?:webp|png|jpeg|gif);base64,[a-z0-9+/=\s]+$/i.test(photo)||/^https:\/\//i.test(photo);
  ...
  function recruitmentDate(value) { return value && Number.isFinite(Date.parse(value)) ? new Date(value).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' }) : 'Não registrada'; }
  function renderApprovedCandidates() {
    const host = $('#rhApprovedList'); if (!host) return;
    const normalize = value => String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
    const query = normalize($('#rhApprovedSearch')?.value.trim()), digits = query.replace(/\D/g, '');
    const rows = recruitmentData.candidates.filter(c => c.hiredUserId && (!query || normalize(c.name).includes(query) || normalize(c.email).includes(query) || normalize(c.phone).includes(query) || (digits.length > 0 && /^[\d\s()+.-]+$/.test(query) && String(c.phone || '').replace(/\D/g, '').includes(digits))));
    host.innerHTML = rows.length ? `<table><thead><tr><th>Candidato</th><th>Contato</th><th>Seleção</th><th>Aprovado em</th><th>Acesso liberado em</th><th>Informações</th></tr></thead><tbody>${rows.map(c => `<tr><td><b>${escapeHtml(c.name)}</b><small>${escapeHtml(c.city || 'Cidade não informada')}</small></td><td>${escapeHtml(c.email || '—')}<small>${escapeHtml(c.phone || '—')}</small></td><td>${escapeHtml(c.experience || 'Experiência não informada')}<small>${c.disc?.completedAt ? `DISC: ${Number(c.disc.result?.match || 0)}%` : 'DISC não realizado'}</small></td><td>${escapeHtml(recruitmentDate(c.approvedAt))}</td><td>${escapeHtml(recruitmentDate(c.accessGrantedAt))}</td><td><button class="rh-card-icon" type="button" data-approved-details="${escapeHtml(c.id)}" title="Ver informações da seleção" aria-label="Ver informações da seleção">${actionIcon('details')}</button>${c.disc?.result ? `<button class="rh-card-icon" type="button" data-approved-disc="${escapeHtml(c.id)}" title="Ver resultado DISC" aria-label="Ver resultado DISC">✓</button>` : ''}</td></tr>`).join('')}</tbody></table>` : '<div class="empty-state">Nenhum candidato encontrado.</div>';
    host.onclick = event => { const button = event.target.closest('[data-approved-details],[data-approved-disc]'); if (!button) return; const c = recruitmentData.candidates.find(c => c.id === (button.dataset.approvedDetails || button.dataset.approvedDisc)); if (c) button.dataset.approvedDisc ? openDiscResult(c) : openRecruitmentDetails(c); };
  }
  ...
  function openRecruitmentDetails(candidate) {
    openSupervisorModal(candidate.name, 'Candidato à vaga', [['WhatsApp', candidate.phone], ['E-mail', candidate.email || '—'], ['Cidade', candidate.city || '—'], ['Experiência', candidate.experience || '—'], ['Currículo', candidate.resumeUrl || '—'], ['Apresentação', candidate.message || '—'], ['Notas da seleção', candidate.notes || '—'], ['Aprovado em', recruitmentDate(candidate.approvedAt)], ['Acesso liberado em', recruitmentDate(candidate.accessGrantedAt)]]);
  ...
  function openDiscResult(candidate) {
    ...
    $('#rhDiscResultBody').innerHTML = `<section class="rh-disc-match"><div><span>${result.commercial?'Índice de respostas comerciais':'Match com a vaga · versão anterior'}</span><b>${Number(result.match || 0)}%</b></div><strong>${escapeHtml(result.predominant || '—')} · ${escapeHtml(labels[result.predominant] || 'Perfil misto')}</strong></section><h3>Composição DISC</h3><div class="rh-disc-bars">${Object.entries(result.scores || {}).map(([key, value]) => `<div><span>${escapeHtml(key)} · ${escapeHtml(labels[key])}</span><i><em style="width:${Number(value)}%"></em></i><b>${Number(value)}%</b></div>`).join('')}</div><h3>Indicadores para a vaga</h3><div class="rh-disc-indicators">${Object.entries(result.indicators || {}).map(([key, value]) => `<article><span>${escapeHtml(indicatorLabels[key] || key)}</span><b>${discLevel(value)} · ${Number(value)}%</b></article>`).join('')}</div><div id="rhDeclineComposer" class="rh-decline-composer" hidden><label><span>Mensagem ao candidato</span><textarea id="rhDeclineMessage" rows="5">Neste momento, seguiremos com outros perfis mais aderentes às necessidades da vaga. Agradecemos seu interesse e desejamos sucesso em sua trajetória profissional.</textarea></label><button id="rhConfirmDeclineBtn" class="btn danger" type="button">Confirmar e enviar e-mail</button></div>`;
    if(result.commercial){
      const section=document.createElement('section');section.className='rh-commercial-result';
      section.innerHTML=`<h3>Situações comerciais</h3><p>Índice baseado nas respostas, não uma previsão de desempenho ou avaliação psicológica validada. Confirme os exemplos na entrevista.</p><div class="rh-disc-indicators">${Object.entries(result.commercial.dimensions).map(([label,value])=>`<article><span>${escapeHtml(label)}</span><b>${Number(value)}%</b></article>`).join('')}</div><h3>Pontos para aprofundar</h3><ul>${result.commercial.interview.map(text=>`<li>${escapeHtml(text)}</li>`).join('')}</ul>`;
      $('#rhDiscResultBody').prepend(section);
    }
  ...
  function renderAdminV2() { renderAdminDashboard(); renderAdminClients(); renderAccessTokens(); renderReceivables(); renderFinancialCalendar(); renderArchivedAdminClients(); refreshAdminListSearches(); }
  ...
  function filterAdminItems(inputSelector, containerSelector, itemSelector, countSelector) {
    const input = $(inputSelector), container = $(containerSelector); if (!input || !container) return;
    const query = normalizedSearchText(input.value); const items = Array.from(container.querySelectorAll(itemSelector)); let visible = 0;
    items.forEach((item) => { const matches = !query || normalizedSearchText(item.textContent).includes(query); item.hidden = !matches; if (matches) visible += 1; });
    const count = $(countSelector); if (count) count.textContent = query ? `${visible} de ${items.length}` : `${items.length} ${items.length === 1 ? 'registro' : 'registros'}`;
  }
  function filterAdminLeadMarketplace() {
    const labels = { users: 'Pesquisar usuário, e-mail, organização ou status', stock: 'Pesquisar lead, contato, perfil, idade, origem ou status', purchases: 'Pesquisar comprador, lead, telefone, perfil ou valor' };
    const input = $('#adminLeadMarketSearch'); if (input) input.placeholder = labels[leadMarketplaceAdminTab] || 'Pesquisar no marketplace';
    filterAdminItems('#adminLeadMarketSearch', '#adminLeadMarketContent', leadMarketplaceAdminTab === 'users' ? '.lead-wallet-card' : 'tbody > tr', '#adminLeadMarketSearchCount');
  }
  function refreshAdminListSearches() {
    filterAdminItems('#adminClientSearch', '#adminClientRows', ':scope > tr', '#adminClientSearchCount');
    filterAdminItems('#adminTokenSearch', '#adminTokenRows', ':scope > .admin-access-group', '#adminTokenSearchCount');
    filterAdminItems('#adminArchivedSearch', '#adminArchivedClientRows', ':scope > tr', '#adminArchivedSearchCount');
    filterAdminLeadMarketplace();
  }
  ...
  function bindAdminMasterEvents() {
    ...
    [['#adminClientSearch', '#adminClientRows', ':scope > tr', '#adminClientSearchCount'], ['#adminTokenSearch', '#adminTokenRows', ':scope > .admin-access-group', '#adminTokenSearchCount'], ['#adminArchivedSearch', '#adminArchivedClientRows', ':scope > tr', '#adminArchivedSearchCount']].forEach(([input, container, items, count]) => $(input)?.addEventListener('input', () => filterAdminItems(input, container, items, count)));
    $('#adminLeadMarketSearch')?.addEventListener('input', filterAdminLeadMarketplace);
  ...
  function bindEvents() {
    ...
    const selectRhTab = approved => {
      $('#rhCandidateKanban').hidden = approved; $('#rhApprovedPanel').hidden = !approved;
      [$('#rhSelectionTab'), $('#rhApprovedTab')].forEach((button, index) => { const active = Boolean(index) === approved; button.classList.toggle('active', active); button.setAttribute('aria-selected', String(active)); });
      if (approved) renderApprovedCandidates();
    };
    $('#rhSelectionTab')?.addEventListener('click', () => selectRhTab(false));
    $('#rhApprovedTab')?.addEventListener('click', () => selectRhTab(true));
    $('#rhApprovedSearch')?.addEventListener('input', renderApprovedCandidates);
