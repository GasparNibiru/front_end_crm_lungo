(() => {
  ...
  function renderApprovedCandidates() {
    ...
    host.innerHTML = rows.length ? `<table><thead><tr><th>Candidato</th><th>Contato</th><th>Seleção</th><th>Aprovado em</th><th>${rhHistoryDismissed ? 'Desligado em' : 'Acesso liberado em'}</th><th>Informações</th></tr></thead><tbody>${rows.map(c => `<tr><td><div class="supervisor-person">${supervisorBrokerAvatar({name:c.name,photo:c.profilePhotoUrl})}<b>${escapeHtml(c.name)}</b></div><small>${escapeHtml(c.city || 'Cidade não informada')}</small></td><td>${escapeHtml(c.email || '—')}<small>${escapeHtml(c.phone || '—')}</small></td><td>${escapeHtml(c.experience || 'Experiência não informada')}<small>${c.disc?.completedAt ? `DISC: ${Number(c.disc.result?.match || 0)}%` : 'DISC não realizado'}</small></td><td>${escapeHtml(recruitmentDate(c.approvedAt))}</td><td>${escapeHtml(recruitmentDate(rhHistoryDismissed ? c.dismissedAt : c.accessGrantedAt))}</td><td><button class="rh-card-icon" type="button" data-approved-details="${escapeHtml(c.id)}" title="Ver informações da seleção" aria-label="Ver informações da seleção">${actionIcon('details')}</button>${c.disc?.result ? `<button class="rh-card-icon" type="button" data-approved-disc="${escapeHtml(c.id)}" title="Ver resultado DISC" aria-label="Ver resultado DISC">✓</button>` : ''}${!c.dismissedAt ? `<button class="rh-card-icon rh-dismiss-btn" type="button" data-approved-dismiss="${escapeHtml(c.id)}" title="Demitir e encerrar acesso" aria-label="Demitir e encerrar acesso">🔥</button>` : !c.dismissalEmailSentAt ? `<button class="tiny-btn" type="button" data-approved-dismiss="${escapeHtml(c.id)}">Reenviar aviso</button>` : '<small>Aviso enviado</small>'}${!c.dismissedAt && c.accessRemovedAt ? '<small>Acesso excluído</small>' : ''}</td></tr>`).join('')}</tbody></table>` : '<div class="empty-state">Nenhum candidato encontrado.</div>';
  ...
  function bindEvents() {
    ...
    const selectRhTab = (approved, dismissed = false) => {
      rhHistoryDismissed = dismissed;
      $('#rhCandidateKanban').hidden = approved; $('#rhApprovedPanel').hidden = !approved;
      [$('#rhSelectionTab'), $('#rhApprovedTab'), $('#rhDismissedTab')].forEach((button, index) => { const active = index === (dismissed ? 2 : approved ? 1 : 0); button.classList.toggle('active', active); button.setAttribute('aria-selected', String(active)); });
    ...
    $('#rhDismissedTab')?.addEventListener('click', () => selectRhTab(true, true));
